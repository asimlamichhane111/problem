...
def orders_view(request, orderId=None):
    if request.method == 'GET':
        ...
        try:
            ...
            if new_status not in ['pending', 'completed', 'cancelled','accepted','rejected']:
            ...
            if new_status == 'completed' and old_status != 'completed':
                ...
                with transaction.atomic():
                    for item in order.items.all():
                        item.product.quantity_sold -= item.quantity
                        item.product.save()
                    # Prevent negative quantity_sold
                        if item.product.quantity_sold < 0:
                            item.product.quantity_sold = 0
                            item.product.save()
